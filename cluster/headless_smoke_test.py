"""Headless Isaac Gym smoke test for the pretrained SimToolReal policy.

This script is intended for short cluster debug runs. It creates a single-env
Isaac Gym instance, loads the pretrained checkpoint, runs a few policy steps,
and exits with a concise summary.
"""
...
def main() -> None:
    ...
    print("[smoke] loading checkpoint and policy")
    ...
    for step_idx in range(args.num_steps):
        ...
        done_value = bool(done[0].item())
        success_value = int(env.successes[0].item())
        object_height = float(env.object_state[0, 2].item())
        print(
            "[smoke] "
            f"step={step_idx + 1}/{args.num_steps} "
            f"reward={reward_value:.4f} "
            f"done={done_value} "
            f"successes={success_value} "
            f"object_z={object_height:.4f}"
        )

    print("[smoke] completed successfully")
    sys.stdout.flush()
    sys.stderr.flush()
    # Isaac Gym can segfault during interpreter teardown after a successful run.
    os._exit(0)
