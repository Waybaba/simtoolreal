...
./scripts/docker_build_isaacgym.sh
```

Run a shell:

```bash
./scripts/docker_run_isaacgym_3090.sh
```

Run training:

```bash
./scripts/docker_run_isaacgym_3090.sh ./scripts/run_isaacgym_simple_marker_3090.sh
```

Run with a custom env count:

```bash
NUM_ENVS=4092 ./scripts/docker_run_isaacgym_3090.sh ./scripts/run_isaacgym_simple_marker_3090.sh
```

Run with camera video enabled:

```bash
CAPTURE_VIDEO=true NUM_ENVS=4092 ./scripts/docker_run_isaacgym_3090.sh ./scripts/run_isaacgym_simple_marker_3090.sh
```

Override isolation:

```bash
CPUSET_CPUS=0-3 MEMORY=24g ./scripts/docker_run_isaacgym_3090.sh
```
